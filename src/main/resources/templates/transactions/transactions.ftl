...
<form class="form-inline" method="post" action="/transactions/save/${transaction.uuid}" id="save_transaction_form">
    <div class="clearfix input-group-sm">
        ...
        <input type="text" class="form-control my-1 mr-sm-2" id="title_input_${transaction.uuid}"
               value="${transaction.title}" name="title" required>

        <label class="sr-only" for="date_input_${transaction.uuid}">Date</label>
        <input type="date" class="form-control my-1 mr-sm-2" id="date_input_${transaction.uuid}"
               value="${transaction.dateField}" name="dateText" required>

        <label class="sr-only" for="amount_input_${transaction.uuid}">Amount</label>
        <input type="number" step="0.01" class="form-control" id="amount_input_${transaction.uuid}"
               value="${transaction.amountString}" name="amount" required>
        ...
        <select class="custom-select my-1 mr-sm-2" id="category_input_${transaction.uuid}" name="category">
            ...
            <#if transaction.category == category.title> selected</#if>>${category.title}</option>
    </
    #list>
    </select>
    ...
    <select class="custom-select my-1 mr-sm-2" id="status_input_${transaction.uuid}" name="status">
        ...
        <#if transaction.status == status.title> selected</#if>>${status.title}</option>
</#list>
</select>

<label class="sr-only" for="description_input_${transaction.uuid}">Description</label>
<input type="text" class="form-control my-1 mr-sm-2" id="description_input_${transaction.uuid}"
       value="${transaction.description}" name="description">
...
<button type="submit" class="btn btn-outline-danger btn-sm rounded"
        formaction="/transactions/delete/${transaction.uuid}">Delete
</button>
</div>
</form>
<#else>
